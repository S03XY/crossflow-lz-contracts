...
describe('OFT testing', function () {
    ...
    it('register domain in chain A for owner A', async function () {
        ...
        // const byte32Address = ethers.utils.formatBytes32String(_domain)
        const byte32Address = ethers.utils.formatBytes32String("aman")

        console.log('byte32domain', byte32Address)

        const tx = await RegisterContractA.connect(ownerA).makeRegisterIntent(byte32Address, { value: 0 })
        const response = await tx.wait()
        if (response.events[0].event === 'EventRegisterIntent') {
            const tx = await RegisterContractA.connect(ownerA).reserveDomain(ownerA.address, byte32Address)
            const response = await tx.wait()
        }

        const registeredDomain = await RegisterContractA.domainToAddress(byte32Address)
